...
/* All eight compressed encodings of edwards25519's order-8 torsion subgroup
 * (the cyclic group <G> for any order-8 point G, i.e. {1G, 2G, ..., 8G=O}).
 * Order 8/gcd(k,8) for kG: orders present are 1 (identity), 2, 4, 4, 8, 8, 8, 8.
 *
 * Not hand-transcribed: derived by decoding an order-8 generator from this
 * file's own unpackneg()/point_add()/point_is_identity(), then enumerating
 * 1G..8G with point_add() and re-encoding with point_pack() -- so their
 * correctness rests on the same curve arithmetic this file already uses for
 * real verification, not on a separately-copied constant that could carry a
 * transcription error. Cross-checked: point k and point (8-k) differ only in
 * the sign bit (byte 31), as required since (8-k)G = -(kG); the order-2
 * element (k=4) is its own negation, as required since 2P = O implies P = -P.
 */
...
/* A handful of distinct messages, so a low-order-R rejection isn't pinned
