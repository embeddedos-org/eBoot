...
/**
 * @file sha512.c
 * @brief SHA-512 (NIST FIPS 180-4) — required by Ed25519 (RFC 8032)
 *
 * Ed25519 as specified in RFC 8032 derives its challenge scalar from
 * SHA-512. A verifier using any other hash cannot check a signature made
 * by a conforming signer, so this primitive is not optional for
 * interoperability with standard tooling.
 *
 * Self-contained, no dynamic allocation, suitable for a bootloader.
 */
...
static void sha512_transform(eos_sha512_ctx_t *ctx)
{
    ...
    for (int i = 0; i < 80; i++) {
        t1 = h + EP1_64(e) + CH64(e, f, g) + K512[i] + w[i];
...
{
    size_t idx = (size_t)(ctx->count % EOS_SHA512_BLOCK_SIZE);

    ctx->count += len;

    while (len > 0) {
        size_t take = EOS_SHA512_BLOCK_SIZE - idx;
        if (take > len) take = len;
        memcpy(ctx->buffer + idx, data, take);
        idx  += take;
        data += take;
        len  -= take;
        if (idx == EOS_SHA512_BLOCK_SIZE) {
...
{
    /* SHA-512 encodes the message length as a 128-bit big-endian bit count.
     * A bootloader never hashes anywhere near 2^61 bytes, so the high 64
     * bits are always zero; they are still written so the padding block is
     * byte-exact against FIPS 180-4. */
    ...
    if (idx > 112) {
        while (idx < EOS_SHA512_BLOCK_SIZE) ctx->buffer[idx++] = 0;
        sha512_transform(ctx);
        idx = 0;
    }
    while (idx < 112) ctx->buffer[idx++] = 0;

    memset(ctx->buffer + 112, 0, 8);              /* high 64 bits of length */
    for (int i = 0; i < 8; i++) {
        ctx->buffer[120 + i] = (uint8_t)(bits >> (56 - 8 * i));
    }
    sha512_transform(ctx);
    ...
    /* Do not leave hash state on the stack of a boot path. */
