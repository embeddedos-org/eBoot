"""eBoot - production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # INTRO
        title = Text("eBoot", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("EoS Bootloader", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        techs = "C, Assembly, ARM, RISC-V".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # FEATURES
        features = [
            ("01", "Secure Boot Chain", "Cryptographic verification of every boot stage from ROM to kernel", DUR["f1"]),
            ("02", "Multi-Architecture Support", "Boots ARM Cortex-M, RISC-V, and x86 targets from a unified codebase", DUR["f2"]),
            ("03", "OTA Firmware Updates", "Atomic over-the-air updates with automatic rollback on failure", DUR["f3"]),
        ]
        ...
        components = ["ROM Stub", "Stage 1", "Stage 2", "Kernel Loader", "OTA Agent"]
        boxes = VGroup()
        ...
        # CTA
        cta_name = Text("eBoot", font_size=72, color=WHITE, weight=BOLD)
        cta_line = Line(LEFT*2, RIGHT*2, color=ACCENT, stroke_width=3)
        cta_line.next_to(cta_name, DOWN, buff=0.3)
        cta_url = Text("github.com/embeddedos-org/eBoot", font_size=22, color=ManimColor(ACCENT))
        cta_url.next_to(cta_line, DOWN, buff=0.3)
        cta_badge = Text("Open Source  |  MIT License  |  Production Ready", font_size=16, color=GRAY_B)
